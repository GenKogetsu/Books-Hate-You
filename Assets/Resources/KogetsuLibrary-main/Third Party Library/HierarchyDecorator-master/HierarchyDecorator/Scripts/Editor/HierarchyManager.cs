...
namespace HierarchyDecorator
{
    ...
    public static class HierarchyManager
    {
        ...
        public static void Initialize()
        {
            ...
            // Unity 6.5+: hierarchyWindowItemOnGUI (int) -> hierarchyWindowItemByEntityIdOnGUI (EntityId)
        ...
        private static void DrawSceneItemHighlight(Rect rect, EntityId id)
        {
            ...
            // Unity 6.5+: Scene.handle is a SceneHandle (wrapper around EntityId), no longer an int
            SceneHandle idAsSceneHandle = SceneHandle.FromRawData(EntityId.ToULong(id));

            for (int i = 0; i < SceneManager.sceneCount; ++i)
            {
                var scene = SceneManager.GetSceneAt(i);
                if (scene.handle == idAsSceneHandle)
                {
